...
def label_encoder_one_layer_v2(decoded_label,
    ...
    dtype = np.float32
    feat_labels = np.zeros(shape, dtype=np.int64)  # 存放預設框匹配的GTbox標籤
    feat_scores = np.zeros(shape, dtype=dtype)  # 存放預設框與匹配的GTbox的IOU（交併比）

    feat_ymin = np.zeros(shape, dtype=dtype)  # 存放預設框匹配到的GTbox的座標資訊
    ...
    for label,box in zip(decoded_label[3],decoded_label[4]):
        ...
        mask = np.greater(IoU, feat_scores)  #每個位置只記錄最大IoU的Box
        imask = mask.astype('int64') #0，1整數  
        fmask = mask.astype('float32')  #0，1浮點數

        feat_labels = imask * label + (1 - imask) * feat_labels  # 當imask為1時更新類別
        feat_scores = np.where(mask, IoU, feat_scores)  # 當mask為true時更新IoU
    ...
    target_dw = np.log(feat_w/wref)/0.2
    target_dh = np.log(feat_h/href)/0.2
    feat_localizations = np.stack([target_dx, target_dy, target_dw, target_dh], axis=-1)
    
    feat_labels = np.reshape(feat_labels,(-1,1))
    feat_localizations = np.reshape(feat_localizations,(-1,4))
    feat_scores = np.reshape(feat_scores,(-1,1))

    
    feat_labels = np.expand_dims(feat_labels, axis=0)
    feat_localizations = np.expand_dims(feat_localizations, axis=0)
    feat_scores = np.expand_dims(feat_scores, axis=0)
...
#     print(target_labels[0].shape,target_labels[1].shape)
    target_labels = np.concatenate(target_labels,axis=1)
    target_localizations = np.concatenate(target_localizations,axis=1)
    target_scores = np.concatenate(target_scores,axis=1)
    
    target_labels = target_labels.astype('float32')
    
    output = np.concatenate([target_labels,target_scores,target_localizations],axis = 2)
...
def label_encoder_one_layer_v3(decoded_label,
                            anchors_layer,
                            image_shape,
                            dtype = np.float32):
    ...
    #從每個annotation中抓出需要的box及label
    for label,box in zip(decoded_label[0],decoded_label[1]):
...
def Label_encoder_one_file_v3(label,
                  image_shape, 
                  layers_shape, 
                  anchor_sizes, 
                  anchor_ratios, 
...
#                                dtype=np.float32)
    ...
    for i, anchors_layer in enumerate(anchors):  # 遍歷每個預測層的預設框
        ...
        t_labels, t_loc, t_scores = label_encoder_one_layer_v3(label, 
                                                               anchors_layer,
                                                               image_shape)
        target_labels.append(t_labels)  # 匹配到的ground truth box對應標籤
        target_localizations.append(t_loc)  # 預設框與匹配到的ground truth box的座標差異
...
def Label_encoder_v3(decode_labels,
                  image_shape, 
                  layers_shape, 
                  anchor_sizes, 
                  anchor_ratios, 
    ...
    for label in decode_labels:
        label_tensor = Label_encoder_one_file_v3(label,
                                              image_shape, 
                                              layers_shape, 
                                              anchor_sizes, 
                                              anchor_ratios, 
